...
GEMINI_API_KEY = "AIzaSyCbK_1jkpJOOgcueS2_bsd9XDCLfhQfvZw"
...
def are_all_inputs_valid(
    ...
    # Remove the API key validation since it's hardcoded
    return True
...
with st.sidebar:
    ...
    if RUN_IN_OFFLINE_MODE:
        ...
        # Add the selectbox to the placeholder (but it won't be displayed)
        with placeholder:
...
def set_up_chat_ui():
    ...
    if prompt := st.chat_input(
        ...
        if not are_all_inputs_valid(prompt, provider, llm_name):
            return
        ...
        try:
            ...
            api_key=GEMINI_API_KEY,  # Use the hardcoded API key
            )
